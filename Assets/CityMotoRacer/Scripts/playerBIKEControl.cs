...
public class playerBIKEControl : MonoBehaviour
{
	...
	public static float moveHorizontal =0;
	private int temp=0;
...
//		}
	...
	void Update ()
	{
		...
		if(GamePlayController.distanceTravelled == 1000){
			End_dis();
	...
	void OnTriggerEnter( Collider c )
	{
		  ...
		  if( c.tag.Contains("coin" ) )//与金钱碰撞触发
		{
		...
		else if( c.GetComponent<Collider>().name.Contains("Magnet" ) )//与磁铁碰撞触发
		{
		...
		else if( c.GetComponent<Collider>().name.Contains("InstantNitrous" ) )//与汽油碰撞触发
		{
			Destroy(c.gameObject);
	...
	void playerIFixedUpdate ()
	{
		...
		if (UIControl.isLeftOn) {
			moveHorizontal=-0.6f;  
		}else if(UIControl.isRightOn){
			moveHorizontal=0.6f;
		}else
			moveHorizontal=0.0f;
		...
		// if(smoothX < -0.5f  && smoothX < 0) 
		// 	smoothX = Mathf.Lerp(smoothX,-1,Time.deltaTime/6);  //smoothX = -1;
		// else if(smoothX > 0.5f  ) 
		// 	smoothX = Mathf.Lerp(smoothX,1,Time.deltaTime/6); 
